...
| TR9 | 跨源融合（FUSE）多条输入 | trust_out ≤ meet(全部输入)，取最脏一环 | 融合取脏（一颗老鼠屎坏一锅汤） | `core/decay.py:98` |

### C 组 · 越格（唯一上升通道：背书门）

| ID | 触发事件 | 变化 | 依据 | 代码位置 |
|---|---|---|---|---|
| TR11 | 越格提升证据：与本地高可信记忆一致 | T ← min(3, T+1)，封顶 T3 | 本地一致性背书 | `core/upgrader.py:183` |
| TR12 | 越格提升证据：结构化校验通过（IOC/CVE/签名） | T ← min(3, T+1)，封顶 T3 | 结构化校验背书 | `core/upgrader.py:183` |
| TR13 | 越格提升证据：人在环显式确认（密码学验签） | T ← T3（直升） | 人在环签名背书 | `core/upgrader.py:183` |
| TR14 | 交叉印证：多个来源需为独立发布实体 | 共享 publisher/ASN 判为 1 源 → 不提升；≥2 独立实体 → T+1 | 抗 Sybil（独立性判到发布实体级） | `core/upgrader.py:151` |
